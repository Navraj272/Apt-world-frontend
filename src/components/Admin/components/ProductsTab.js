    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Manage physical products and machinery options</p>
        </div>
        <button
                      ...
                      <div className="flex flex-wrap gap-1.5 max-w-xs">
                        {product.specs && Object.keys(product.specs).length > 0 ? (
                          Object.entries(product.specs).map(([key, val]) => (
                            <span key={key} className="bg-[#050a16] text-slate-400 border border-slate-800 px-1.5 py-0.5 rounded-[3px] text-[10px] lowercase font-medium">
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#091225] border border-slate-800 rounded-sm w-full max-w-2xl shadow-2xl overflow-hidden animate-zoom-in max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto flex-grow p-6 space-y-6">
              ...
              {/* Modal Actions */}
              <div className="flex justify-end space-x-3 pt-6 border-t border-slate-800 flex-shrink-0 mt-6">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
