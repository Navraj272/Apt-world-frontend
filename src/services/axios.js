import { getAccessToken, removeLoginToken,deleteFromSessionStorage } from './storageUtils';
...
const handleResponse = async (response) => {
  if (!response.ok) {
    ...
    if (status === 401 || status === 403) {
      if (getAccessToken()) {
        removeLoginToken(true);
        deleteFromSessionStorage('popupSent');
      }
...
const makeRequest = async (url, method, data = {}, config = {}, params = {}) => {
  const headers = {
   ...
   'user-location': sessionStorage.getItem('locationData'),
    ...config,
  };
